...
\usepackage{xfrac}
\usepackage[pdftex,
            pdfauthor={William Chen, Pablo Angulo},
            pdftitle={Probability Cheatsheet},
            pdfsubject={A cheatsheet pdf and reference guide originally made for Stat 110, Harvard's Introduction to Probability course. Formulas and equations for your statistics class. Modified by Pablo Angulo for ETSIN@UPM},
...
\lstset{ frame=Ltb,
     framerule=0pt,
     aboveskip=0.18cm,
     framextopmargin=0.2pt,
     framexbottommargin=0.4pt,
...
Original work at \url{http://github.com/wzchen/probability_cheatsheet},
modified by Pablo Angulo for ESTIN@UPM.ES \url{https://github.com/pablo-angulo/probability_cheatsheet}
...
\subsection{Sampling Table}
   \begin{minipage}{\linewidth}
    ...
    \subsection{Cardano's Definition of Probability}  {If the number of outcomes is finite and all outcomes are equally likely}, the probability of an event $A$ happening is:
        \[P_{\textrm{Cardano}}(A) = \frac{\textnormal{number of outcomes favorable to $A$}}{\textnormal{number of outcomes}}\]

\section{Set algebra} \smallskip \hrule height 2pt \smallskip
...
\section{Probability} \smallskip \hrule height 2pt \smallskip

\subsection{Axioms of probability}
Any assignment from subsets of $E$ to real numbers is a \textit{probability measure} if the following holds:
    \begin{description}
        \item[Probabilities are positive]  $P({A}) \geq 0$.
        \item[The probability of the whole space is $1$]  $P({E}) = 1$.
        \item[Probabilities of a union of disjoint sets]  $P(A\cup B) = P(A) + P(B)$, \textit{provided} $A\cap B=\emptyset$.
    \end{description}

\subsection{Consequences}
For any probability measure, the following are true:
    \begin{description}
        \item[Probability of the empty set]  $P(\emptyset) = 0$.
        \item[Probability of the complement]  $P(A^C) =1 - P(A)$.
    \end{description}
...
$$
\begin{array}{rl}
 P(B)&=P(B|A_{1})P(A_{1})+\ldots +P(B|A_{n})P(A_{n})\\
     &=\sum _{i=1}^{n}P(B|A_{i})P(A_{i})
\end{array}
$$
...
\subsection{Independence}
    ...
    \begin{description}
        ...
        %   \end{align*}
        \item[2 Independent Events] $A$ and $B$ are independent if knowing whether $A$ occurred gives no information about whether $B$ occurred. More formally, $A$ and $B$ (which have nonzero probability) are independent if and only if one of the following equivalent statements holds:
...
%\texit{All} of the following statements hold:
...
\section{Continuous Random Variables (CRVs)}\smallskip \hrule height 2pt \smallskip
...
\section{LOTUS,  UoU}\smallskip \hrule height 2pt \smallskip
...
\section{Continuous Multivariate Distributions} \smallskip \hrule height 2pt \smallskip

    \begin{description}
        \item[Joint Probability density] $f(x,y)\leadsto P((X,Y)\in A)=\int_A f(x, y)$.
%        \item[Marginal (Unconditional) Probability] $P({A})$ -- Probability of ${A}$.
        \item[Marginal density]  $f_x(x)=\int_{\mathbb{R}}f(x,y)\;d y\leadsto P(X\in C)=\int_C f_x(x)\; d x$.
    \end{description}
...
A  vector $\vec{X} = (X_1, X_2, \dots, X_d)$ is  Multivariate Normal if every linear combination is Normally distributed, i.e., $t_1X_1 + t_2X_2 + \dots + t_dX_d$ is Normal for any constants $t_1, t_2, \dots, t_d$. The parameters of the Multivariate Normal are the \textbf{mean vector} $\vec{\mu} = (\mu_1, \mu_2, \dots, \mu_d)$ and the \textbf{covariance matrix} $\Sigma$ where the $(i, j)$ entry is $\cov(X_i, X_j)$.
...
\item The joint PDF of a Multivariate Normal is:
$f(x)= \operatorname {det} ((2\pi )^{d}{\boldsymbol {\Sigma }})^{-{\frac {1}{2}}}\,e^{-{\frac {1}{2}}(\mathbf {x} -{\boldsymbol {\mu }})'{\boldsymbol {\Sigma }}^{-1}(\mathbf {x} -{\boldsymbol {\mu }})}$
...
\section{Maximum likelihood} \smallskip \hrule height 2pt \smallskip

The RV $X$ follows a parametric distribution $X\sim \mathcal{D}(\lambda)$.
We don't know $\lambda$, but we have $n$ independent observations $\{x_j\}_{j=1}^n$ from $X$.
The \textbf{likelihood} of $\lambda$ is:
\begin{itemize}
  \item
...
$$
\end{itemize}
The \emph{maximum likelihood estimator} of $\lambda$ is the value $\lambda$* that maximizes the likelihood:
$$
\lambda^* = \operatorname{argmax}_{\lambda} \mathcal{L}\left(\lambda|\:(x_j)\right)
$$

\section{Conjugate families} \smallskip \hrule height 2pt \smallskip
In the Bayesian approach to statistics, parameters are uncertain, so we assign a probability distribution to them. The \textbf{prior} for a parameter is its distribution before observing data. The \textbf{posterior}  is the distribution for the parameter after observing data.
...
The \textbf{Jeffreys prior} is invariant by reparameterization: the Jeffreys prior for $\sigma$, $\sigma^2$ and $\tau$ are all equivalent.

For a single parameter, the Jeffreys prior is also a \textbf{reference prior}: it maximizes the expected information gain from the data.

Jeffreys prior for:
\begin{description}
  \item [a probability $p$:] density $f(p)\propto \frac{1}{\sqrt{p(1-p)}}$: the $\Beta(\frac{1}{2},\frac{1}{2})$ distribution.
  \item [the rate $\lambda$ of a Poisson process:] ``pseudo-density'' $f(\lambda)\propto \frac{1}{\sqrt{\lambda}}$: the $\Gam(\frac{1}{2},0)$ distribution.
  \item [the mean $\mu$ of a Normal $\N$ experiment with known precision $\tau$:] ``pseudo-density'' $f(\mu)\propto 1$: the uniform distribution on $\R$, which is \emph{improper}, and is also the $\N(\mu=0,\tau=0)$ distribution.
  \item [the precision $\tau$ of Normal $\N$ experiment with known mean $\mu$:]  ``pseudo-density'' $f(\tau)\propto \frac{1}{\tau}$: the $\Gam(0,0)$ distribution for the precision $\tau$.
\end{description}

\section{Regression}
\textbf{General regression model}: some variables $X_1,\dots, X_n$ are known (not random), others $\varepsilon_1, \dots,\varepsilon_k$ are random.
The goal is to understand better, and make predictions for the target variable $Y$.
The function $f$ is unknown:
$$
Y = f(X_1,\dots, X_n, \varepsilon_1, \dots,\varepsilon_k)
$$
\subsection{Linear Regression}
\begin{align*}
  Y &=  f(X_1,\dots, X_n, \varepsilon) \\
  &= \beta_0 +\beta_1\cdot X_1 + \dots +\beta_n\cdot X_n + \varepsilon\\
  &\varepsilon\sim\N(0,\sigma^2).
\end{align*}
\textbf{Least squares} regression: mininize $RSS = \sum (y_j - f(x_j))^2$.
It's also the maximum likelihood estimation for the unknown parameters $\beta_0,\dots,\beta_n,\sigma$.

\section{Software} \smallskip \hrule height 2pt \smallskip
\subsection{\texttt{scipy.stats}}
\begin{description}
  \item[A frozen distribution] \texttt{N = scipy.stats.norm(loc=mean, scale=std)}
  \item[Random sample of size \texttt{N}] \texttt{N.rvs(N)}
  \item[Mean] \texttt{N.mean()}
  \item[Variance] \texttt{N.var()}
  \item[Distribution function at points \texttt{xs} (array)] \texttt{N.cdf(xs)}
  \item[Density function at points \texttt{xs} (if continuous)] \texttt{N.pdf(xs)}
  \item[Mass function at points \texttt{xs} (if discrete)] \texttt{N.pmf(xs)}
  \item[Percentiles \texttt{ps}] \texttt{N.ppf(ps)}
\end{description}
...
Create a dataframe:
{\tiny
\lstinputlisting[language=python]{pandas.py}
}
% \begin{verbatim}
...
%     "probability": [7,6,6,8],
%     },
%     index = ["Jaimita", "Fulanito", "Menganito", "Zutanita"],
% )
% \end{verbatim}
\begin{description}
...
{\tiny
\lstinputlisting[language=python]{sklearn1.py}
}
% \begin{verbatim}
...
Make predictions
{\tiny
\lstinputlisting[language=python]{sklearn2.py}
}
% \begin{verbatim}
...
{\tiny
\lstinputlisting[language=python]{sklearn3.py}
}
% \begin{verbatim}
...
{\tiny
\lstinputlisting[language=python]{statsmodels.py}
}
% \begin{verbatim}
...
\end{description}
% \begin{description}
%   \item[Linear regression] \texttt{regr = skl_lm.LinearRegression()}
%   \item[Fit model] \texttt{regr.fit(X_train, y_train)}
% \end{description}
...
\section{Computations with one dimensional Random Variables}
\begin{center}
\renewcommand{\arraystretch}{3.7}
\begin{tabular}{cccccc}
\textbf{Object of interest} & \textbf{Finite} & \textbf{Infinite discrete}  & \textbf{Continuous} & \textbf{Sample}\\
\hline
\textbf{Support} & A finite set $F$ & \shortstack{An infinite but countable set $I$\\ e.g. $\mathbb{N}, \mathbf{Z}, \dots$} & \shortstack{ A subset $S$ of $\mathbb{R}$\\ e.g. $\mathbb{R}, (0,\infty), (a,b)$}& \shortstack{A sample of size $N$\\$\{x_1,\dots,x_N\}$ }\\
\hline
\hline
\shortstack{$P(X\in A)$\\\textbf{probability} of $A$}
& \shortstack{$\displaystyle\sum_{k\in A\cap F}p_X(k)$ \\$p_X$ is the \textbf{mass function}}
& \shortstack{$\displaystyle\sum_{k\in A\cap I}p_X(k)$ \\$p_X$ is the \textbf{mass function}}
& \shortstack{$\int_{A\cap S}f_X(x)\:d x$ \\$f_X$ is the \textbf{density function}}
%& $P_{\textrm{sample}}(A) = \frac{\#\{ x_i\in A \}}{ N }$
& \shortstack{$P(A)\approx P_{\textrm{sample}}(A)$ \\ $P_{\textrm{sample}}(A) = \frac{\textnormal{number of $x_i$ that lie in $A$}}{ N }$}\\
%\hline
% \shortstack{$P(X\leq t)$} & \shortstack{$F_X(t)$ \\$F_X$ is the \textbf{distribution function}}
% & $F_X(t)$
% & $F_X(t)$
% & $F_{\textrm{sample}}(A) = \frac{\textnormal{number of $x_i$ smaller than $t$}}{ N }$\\
\hline
\shortstack{$P(X\leq t)$} &
\multicolumn{3}{c}{
\shortstack{$F_X(t)$ \\$F_X$ is the \textbf{distribution function}}}
& \shortstack{$P(X\leq t)\approx F_{\textrm{sample}}(A)$\\$F_{\textrm{sample}}(A) = \frac{\textnormal{number of $x_i$ smaller than $t$}}{ N }$ \\ faster to compute if the sample is ordered}\\
\hline
\hline
\shortstack{$g(X)$\\\textbf{transformation }of $X$ by $g$\\ $g$ is \emph{inyective}}
...
& \shortstack{$\displaystyle\sum_{k\in I}g(k)\:p_X(k)$ \\an infinite \textbf{series}}
& \shortstack{$\int_{S}g(x)\:f_X(x)\:d x$ \\an \textbf{integral}}
%& $P_{\textrm{sample}}(A) = \frac{\#\{ x_i\in A \}}{ N }$
& $E[g(X)]\approx \frac{\Sigma_{i=1}^N g(x_i)}{ N }$\\
\hline
\hline
\shortstack{$X + Y$\\\textbf{sum} of RVs $X$ and $Y$}
& \multicolumn{3}{c}{
\shortstack{can get complicated (involves ``convolutions'')\\ except in a few special cases}
}
%& $P_{\textrm{sample}}(A) = \frac{\#\{ x_i\in A \}}{ N }$
& \shortstack{$\{x_1+y_1,\dots,x_N+y_N\}$ \\ is a sample of $X+Y$ of size $N$ }\\
\hline
\shortstack{$X$ follows a parametric distribution\\ $X\sim \mathcal{D}(Y,Z,\dots)$ \\the parameters $Y,Z,\dots$ are RVs}
& \multicolumn{3}{c}{
\shortstack{rather complicated except in a few special cases}
}
%& $P_{\textrm{sample}}(A) = \frac{\#\{ x_i\in A \}}{ N }$
& \shortstack{first sample $y_j\in Y,z_j\in Z,\dots$\\ then sample $x_j$ from $\mathcal{D}(y_j,z_j,\dots)$ \\ $\{x_1,\dots,x_N\}$ is a sample of $X$ of size $N$ }\\
\hline
\hline
\end{tabular}
\end{center}
...
\shortstack{Bernoulli \\ \Bern($p$)} & \shortstack{$P(X=1) = p$ \\$ P(X=0) = q=1-p$} &
 % $\begin{cases}0&{\text{if }}p<1/2\\0,1&{\text{if }}p=1/2\\1&{\text{if }}p>1/2\end{cases}$ &
$p$ & $pq$ & \texttt{bernoulli(p=p0)}\\
\hline
\shortstack{Binomial \\ \Bin($n, p$)} & \shortstack{$P(X=k) = {n \choose k}p^k q^{n-k}$  \\ $k \in \{0, 1, 2, \dots n\}$}&
% \shortstack{$\lfloor (n+1)p\rfloor $ \\ or $\lceil (n+1)p\rceil -1$}&
...
\hline
%\shortstack{Negative Binomial \\ \NBin($r, p$)} & \shortstack{$P(X=n) = {r + n - 1 \choose r -1}p^rq^n$ \\ $n \in \{$0, 1, 2, \dots $\}$} & $rq/p$ & $rq/p^2$ \\
%\hline
%\shortstack{Hypergeometric \\ \Hypergeometric($w, b, n$)} & \shortstack{$P(X=k) = \sfrac{{w \choose k}{b \choose n-k}}{{w + b \choose n}}$ \\ $k \in \{0, 1, 2, \dots,  n\}$} & $\mu = \frac{nw}{b+w}$ &$\left(\frac{w+b-n}{w+b-1} \right) n\frac{\mu}{n}(1 - \frac{\mu}{n})$\\
%\hline
...
%\hline
%\shortstack{Log-Normal \\ $\mathcal{LN}(\mu,\sigma^2)$} & \shortstack{$f(x)=\frac{1}{x\sigma \sqrt{2\pi}}e^{-(\log x - \mu)^2/(2\sigma^2)}$\\$x \in (0, \infty)$} & $\theta = e^{ \mu + \sigma^2/2}$ & $\theta^2 (e^{\sigma^2} - 1)$ & \texttt{lognorm}\\
% \hline
% \shortstack{Weibull \\ $\mathcal{W}(k,\lambda)$} & \shortstack{$f(x)=\frac {k}{\lambda }\left({\frac {x}{\lambda }}\right)^{k-1}e^{-(x/\lambda )^{k}}$\\$x \in (0, \infty)$} & $ \lambda\Gamma(1+1/k)$ & $\lambda^2\left[\Gamma\left(1+\frac{2}{k}\right) - \left(\Gamma\left(1+\frac{1}{k}\right)\right)^2\right]\,$ & \texttt{weibull\_min(c=k,scale=lambd)}\\
% \hline
% \shortstack{Chi-Square \\ $\chi_n^2$} & \shortstack{$f(x)=\frac{1}{2^{n/2}\Gamma(n/2)}x^{n/2 - 1}e^{-x/2}$\\$x \in (0, \infty) $} & $n$  & $2n$ & \texttt{chi2}\\
\hline
\hline
\shortstack{Multivariate Normal \\ $\N(\mathbf{\mu}, \Sigma)$} &
%\shortstack{$f(x) = \frac{1}{\sigma \sqrt{2\pi}} e^{-\sfrac{1}{2}(x - \mathbf{\mu})'\Sigma^{-1}(x - \mathbf{\mu})}$ \\ $x \in \mathbb{R}^N$}
...
\\\hline
  ...
  \multicolumn{2}{l}{
  \shortstack{$\hat{\mathbf{\mu}}=\bar{\mathbf{x}}=\frac{1}{n}\sum_{j=1}^n \mathbf{x}_j$ \\
              $\hat{\Sigma}=\frac{1}{n}\sum_{j=1}^n (\mathbf{x}_j-\bar{\mathbf{x}})\cdot (\mathbf{x}_j-\bar{\mathbf{x}})^t $}} &
  \multicolumn{2}{c}{... \href{https://en.wikipedia.org/wiki/Multivariate\_normal\_distribution\#Bayesian\_inference}{$\operatorname{Normal-Wishart}$} ...}
  % \shortstack{$\mu=\frac{\nu m + n \bar{x}}{\nu +n}$ \\ $\sigma^2= ... $}
  \\\hline
\end{tabular}
\end{center}
